  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-28">
      <div className="text-center">
        ...
        <p className="mt-3 text-[48px] font-extrabold tracking-tight text-[#3d2b1f] sm:text-[56px]">
          {socialProof.metric}
        </p>
      ...
      <div className="mt-16 grid gap-6 sm:grid-cols-2">
        {socialProof.quotes.map((quote) => (
          <div
            key={quote.author}
            className="rounded-xl border border-[#e8ddd0]/70 border-l-[3px] border-l-[#c85a28]/40 bg-white p-7 shadow-card"
          >
            <div className="flex items-center gap-0.5">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className="size-3.5 fill-[#c85a28] text-[#c85a28]"
                />
              ))}
            </div>
